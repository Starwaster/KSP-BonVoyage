...
namespace BonVoyage
{
	public class PathUtils
	{
		...
		internal static string EncodePath(Path<Hex> path)
		{
			...
			// Replace forward slash with # (two forward slashes are seems to be interpreting as start of the comment, when read from a save file)
		...
		internal static List<Vector3d> DecodePath(string pathEncoded, CelestialBody body)
		{
			...
			if (!pathEncoded.Contains(";"))
			{
				...
				// Change LZString implementation of base64 to native functions
				// Replace # with forward slash (two forward slashes are seems to be interpreting as start of the comment, when read from a save file)
